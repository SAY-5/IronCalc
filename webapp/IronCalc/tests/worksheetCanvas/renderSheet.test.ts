...
test("merged range over hidden rows and columns", async () => {
  const model = await newModel();
  // B2:D4 merged; column C and row 3 are hidden, so the merged rectangle
  // must shrink and still render as one continuous block
  model.setUserInput(0, 2, 2, "Merge over hidden");
  model.updateRangeStyle(cell(2, 2, 3, 3), "fill.color", "#FCE5CD");
  model.updateRangeStyle(cell(2, 2, 3, 3), "alignment.horizontal", "center");
  model.mergeCells(cell(2, 2, 3, 3));
  model.setColumnsWidth(0, 3, 3, 0);
  model.setRowsHeight(0, 3, 3, 0);

  await expectScreenshot(await renderToCanvas(model), "merged-over-hidden");
});
